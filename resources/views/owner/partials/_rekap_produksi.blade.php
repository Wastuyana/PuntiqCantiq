<div class="bg-white p-6 rounded-lg shadow-sm border border-slate-100 mb-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
        <div class="p-4 bg-slate-50 rounded-xl border border-slate-100">
            <p class="text-[10px] font-bold text-slate-400 uppercase mb-1">Total Target Output</p>
            <h4 class="text-2xl font-black text-slate-600">{{ number_format($totalTarget) }} <span
                    class="text-xs font-normal">Bks</span></h4>
        </div>

        <div class="p-4 bg-primary/5 rounded-xl border border-primary/20">
            <p class="text-[10px] font-bold text-primary uppercase mb-1">Total Realisasi Hasil</p>
            <h4 class="text-2xl font-black text-primary">{{ number_format($totalAktual) }} <span
                    class="text-xs font-normal">Bks</span></h4>
        </div>

        <div
            class="p-4 {{ $selisihKumulatif < 0 ? 'bg-error/5 border-error/20' : 'bg-success/5 border-success/20' }} rounded-xl border">
            <p class="text-[10px] font-bold {{ $selisihKumulatif < 0 ? 'text-error' : 'text-success' }} uppercase mb-1">
                Total Selisih (Waste)</p>
            <h4 class="text-2xl font-black {{ $selisihKumulatif < 0 ? 'text-error' : 'text-success' }}">
