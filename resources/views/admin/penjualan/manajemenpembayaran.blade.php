    ...
    <div class="p-6 bg-base-100 rounded-lg shadow" 
        ...
        <div class="flex p-1 bg-base-200 rounded-lg w-fit mb-6">
            <button @click="tab = 'hutang'; window.history.pushState({}, '', '?tab=hutang')" 
                    ...
                    class="px-6 py-2 rounded-md font-bold text-sm transition">Piutang Aktif</button>
            <button @click="tab = 'lunas'; window.history.pushState({}, '', '?tab=lunas')" 
                    :class="tab === 'lunas' ? 'bg-success text-white' : ''" 
                    class="px-6 py-2 rounded-md font-bold text-sm transition">Riwayat Lunas</button>
        </div>
            ...
            <table class="table w-full text-xs">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $row->status_customer == 'mitra' ? ($row->mitra->nama_mitra ?? 'Mitra') : ($row->pelanggan->nama_pelanggan ?? 'Pelanggan') }}</td>
    ...
    <dialog id="modal_lunas_{{ $row->id }}" class="modal">
        <div class="modal-box">
            ...
            <form action="{{ route('admin.penjualan.manajemenpembayaran.lunasi', $row->id) }}" method="POST">
                ...
                <select name="metode_pembayaran" class="select select-bordered w-full mb-4" required>
                ...
                <div class="modal-action">
                    <button type="submit" class="btn btn-success text-white">Set Lunas</button>
